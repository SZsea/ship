...
uniform float rotation_speed : hint_range(0.0, 100.0) = 0.5;
...
uniform int OCTAVES : hint_range(0, 20, 1);//地形锯齿
...
void fragment() {
	...
	vec2 base_fbm_uv = (uv)*size+ vec2(time*0.04,0.0);
	...
	d_light = distance(uv , newlight_origin);
	
	// use multiple fbm's at different places so we can determine what color land gets
	float fbm1 = fbm(base_fbm_uv);
	float fbm2 = fbm(base_fbm_uv - newlight_origin*fbm1);
	float fbm3 = fbm(base_fbm_uv - newlight_origin*1.5*fbm1);
	float fbm4 = fbm(base_fbm_uv - newlight_origin*2.0*fbm1);
